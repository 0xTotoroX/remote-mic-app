...
本轮未在用户现场复测 macOS Cmd-Tab UI；必须使用真实 A2540/A2854 完成：顺/逆时针切换、圆周轻点确认、实体确定/返回、超时、断连，并核对 `selection_observed result=changed|unchanged`。自动化中的 `success=true` 仍不能替代系统前台变化验收。
