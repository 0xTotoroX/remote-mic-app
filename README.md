...
不同遥控器的收音方式不同：小米遥控器只能按住收音；Chromecast 语音遥控器既支持按住，也支持「按一下开始、再按一下结束」；Siri Remote 另有触摸面。Mac 端的语音工具同样分两类：有的按住说话，有的按一次切换。**两边不是同一类时会出现「按一下结束没生效、要再按一下」或「按下没说几句就结束」。**
