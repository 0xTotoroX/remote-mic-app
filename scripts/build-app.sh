...
# Accept the correctly spelled public names used by local-package tooling while
# retaining the historical internal Chromecase names for existing callers.
if [[ -n "${REQUIRE_SAYALL_CHROMECAST:-}" ]]; then
  REQUIRE_SAYALL_CHROMECASE="$REQUIRE_SAYALL_CHROMECAST"
fi
if [[ -n "${SAYALL_CHROMECAST_PACKAGE_PATH:-}" ]]; then
  SAYALL_CHROMECASE_PACKAGE_PATH="$SAYALL_CHROMECAST_PACKAGE_PATH"
fi
