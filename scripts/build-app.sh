...
SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH="${SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH:-}"
...
if [[ -n "${SAYALL_MEMBERSHIP_PACKAGE_PATH:-}" ]]; then
  print -u2 "SAYALL_MEMBERSHIP_PACKAGE_PATH is no longer supported; use SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH"
  exit 1
fi
if [[ -n "$SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH" ]]; then
  if [[ ! -f "$SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH/Package.swift" ]]; then
    print -u2 "SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH must contain Package.swift"
    exit 1
  fi
  SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH="${SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH:A}"
  export SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH
  SAYALL_MEMBERSHIP_ADAPTER_INCLUDED=true
else
  SAYALL_MEMBERSHIP_ADAPTER_INCLUDED=false
fi
if [[ -n "$SAYALL_PRIVATE_ARTIFACT_PACKAGE_PATH" ]]; then
  if [[ -n "$SAYALL_COMBINATION_ACTIONS_PATH" || -n "$SAYALL_MEMBERSHIP_ADAPTER_PACKAGE_PATH" ]]; then
...
if [[ "$SAYALL_MEMBERSHIP_ADAPTER_INCLUDED" == "true" ]]; then
  SCRATCH_FLAVOR="${SCRATCH_FLAVOR}-membership-adapter"
fi
