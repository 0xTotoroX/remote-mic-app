...
预期结果：无需重启 App，重连后的第一枚按键和第一次语音会话即可工作。多只 Apple Remote 同时连接时，每只设备都能直接触摸，不需要先按实体键；新增或断开设备会刷新触摸源，已有活动 contact 明确取消，剩余设备随后可继续触摸。
...
5. 同时连接两只 Siri Remote，分别直接触摸（不先按实体键）、交替触摸，再断开其中一只后继续触摸；确认每只遥控器都能独立产生触摸生命周期。
6. 回放外圈向中心的径向轨迹、外圈圆周轨迹和阈值附近抖动；确认单次 contact 只锁定一种模式。
7. 检查触摸移动时主题色光环完整围绕原生光标主体；原生箭头、I-Beam、链接手型和窗口缩放光标仍由系统正常显示。光环按速度平滑缩放，不在光标旁边出现第二个独立图标。
8. 顺时针和逆时针转动外圈，确认光环右侧缺口只显示页面滚动指示器的向上或向下箭头；慢速为 1 个、正常为 2 个、快速为 3 个。分别选择“与页面滚动指示器一致”和“相反”，确认只反转箭头，不改变页面实际滚动方向、自然滚动设置或 iPod 转盘方向。
9. 在支持 `AXPress` 的标准按钮或链接上停止移动，确认光环延长约 2 秒；期间分别测试实体 OK 和中心区域短促静止轻触，两种方式都只激活目标一次。中心轻触不得执行 OK 的键位映射，也不得额外产生第二次鼠标点击。
10. 在确认窗口内移动光标、开始滚动、等待超时、开始语音、断开遥控器，并分别再次触摸或按 OK，确认旧目标已取消且普通输入恢复。
