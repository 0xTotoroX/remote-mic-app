...
预期结果：指针移动稳定且相较旧包明显更快、轻触只点击一次、外圈可以连续滚动且步幅足够；低速滚动可精确控制，正常到快速旋转时页面速度单调增加且不突跳。外圈向中心的径向轨迹锁定为鼠标移动，累计圆周轨迹达到阈值后锁定为滚动，同一次接触不会在两种模式之间切换。移动和滚动反馈围绕原生鼠标且不覆盖点击热点；两种反馈大小一致，并按相同速率随速度放大或缩小；滚动缺口箭头清晰表示页面上下方向，数量随速度从 1 到 3 单调增加。鼠标停在公开 Accessibility API 报告自身或最多四层父元素支持 `AXPress` 的目标上时，指示器延长显示，延长期内一次 OK 只触发一次该元素；滚动、触摸点击、超时、断连或 App 停止都会取消待点击目标。Cmd+Tab 显示期间不移动鼠标、不触发普通点击或垂直滚动；顺时针转圈稳定选择右侧/下一项，逆时针选择左侧/上一项；在圆周模式静止轻点应确认当前 App，不发送鼠标点击；退出切换器后下一次触摸恢复普通指针与滚动。触摸结束后不漂移。日志应记录 `phase=selection_observed`，并明确 `result=changed` 或 `result=unchanged`。
