...
预期：登录态由安全会话恢复；已登录时个人中心入口显示邮箱 `@` 前最多 6 个字符，前缀不足 6 个字符时完整显示；未登录或退出后显示“登录”；刷新保留用户名，退出立即清空用户名；退出后本机立即失效，服务端撤销失败时可在网络恢复后补偿；Free 显示升级入口，Plus 显示有效权益。日志和可复制诊断不得包含邮箱、验证码、Token、安装标识或服务响应正文。
